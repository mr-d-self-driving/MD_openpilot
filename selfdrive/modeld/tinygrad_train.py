...
def train(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print(f"📝  saved {ckpt}")
    out_onnx = Path(args.onnx).with_suffix("_ty_finetuned.onnx")
    dump_weights_back_into_onnx(runner, model_proto, str(out_onnx))
...
def dump_weights_back_into_onnx(runner: OnnxRunner,
                                ...
                                path_out: str):
    """
    Overwrite every initializer in model_proto with the *current* tensor data
    from runner.graph_values and write the result to `path_out`.
    """
