...
P.add_argument("--epochs", type=int,   default=1,    help="Fine-tuning epochs")
...
if args.save_onnx:
    ...
    save_path = Path(args.save_onnx).expanduser().resolve()
    save_path.parent.mkdir(parents=True, exist_ok=True)

    # --- Build lookup tables ------------------------------------------------
    init_map     = {init.name: init for init in onx.graph.initializer}
    shape_to_inits = defaultdict(list)
    for name, init in init_map.items():
        shape_to_inits[tuple(init.dims)].append(name)

    def try_aliases(pname: str) -> List[str]:
        """Return possible ONNX names for a given PyTorch parameter name."""
        base = pname.lstrip("model.")            # drop 'model.' if present
        aliases = {
            base.replace(".", "/"),              # Gemm_12.weight -> Gemm_12/weight
            base,                                # Gemm_12.weight
            pname.replace(".", "/"),             # full path w/ slashes
        }
        return [a for a in aliases if a in init_map]

    updated, fuzzy, missed = 0, 0, 0
    for pname, param in model.named_parameters():
        if not param.requires_grad:
            continue

        # 1.  Exact / alias search
        aliases = try_aliases(pname)
        if aliases:
            onnx_name = aliases[0]               # first match wins
            init_map[onnx_name].CopyFrom(
                nph.from_array(param.detach().cpu().numpy(), name=onnx_name)
            )
            updated += 1
            continue

        # 2.  Shape-only fallback (unique shape ⇒ safe to map)
        candidates = shape_to_inits[tuple(param.shape)]
        if len(candidates) == 1:
            onnx_name = candidates[0]
            init_map[onnx_name].CopyFrom(
                nph.from_array(param.detach().cpu().numpy(), name=onnx_name)
            )
            fuzzy += 1
        ...
        missed += 1      # couldn’t find a unique target

    print(
        f"✓ {updated} tensors updated by name, "
        f"{fuzzy} by unique-shape, {missed} still unmatched"
    )

    onnx.save(onx, save_path.as_posix())         # cast to str for good measure
    print("✓ Finetuned ONNX saved →", save_path)